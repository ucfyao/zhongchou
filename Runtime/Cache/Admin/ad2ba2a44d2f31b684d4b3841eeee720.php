...
<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$zml): $mod = ($i % 2 );++$i;?><div class="x12 bg padding">
		...
		<div class="x9 bg padding bg-white border-left height-big">
			...
			<div class="x12">
				...
				<a href="javascript:void(0)" onclick="iframe('/index.php/Admin/Item/interview/itemid/<?php echo ($zml["id"]); ?>')" class="button button-small bg-sub radius-none"><?php echo ($zml["interview"]); ?> 位约谈</a>
			...
			<div class="x12 text-gray">
				...
				<div class="x6">
					...
					<span class="x6">项目发布者: <span class="text-red"><a href="/index.php/Admin/Item/index/uin/<?php echo ($zml["uin"]); ?>" class="text-sub"><?php echo ($zml["user_name"]); ?></a></span></span>
				</div>
			</div>
		</div>
...
<script>
	...
	function lead(itemid,b){
		$.get("/index.php/Admin/Lead/lead_list/itemid/"+itemid, function(d){
			if (d.status==1) {
	...
	function sub(a){
		...
		$.post("/index.php/Admin/Lead/sub_lead", {
		 	itemid: itemid.val(),
