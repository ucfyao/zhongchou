...
layer.config({
    ...
    <div class="x12 padding-top padding border-bottom">
		...
		<a href="javascript:void(0)" class="float-right bg-sub button icon-plus" type="button" onclick="iframe('<?php echo U('Admin/Ads/ads_edit');?>')"> 添加幻灯片</a>
      </div>
        ...
        <table class="table table-bordered table-hover">
          <tbody>
            ...
            <?php if(is_array($lists)): $i = 0; $__LIST__ = $lists;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr id="tr<?php echo ($vo["id"]); ?>" class="height">
              <td><img src="<?php echo ($vo["img"]); ?>" width="100" /></td>
              ...
              <td>
                ...
                <a href="javascript:void(0)" class=" button button-small bg-sub" onclick="iframe('<?php echo U('Admin/Ads/ads_edit',array('id'=>$vo['id']));?>')" >修改</a>
              </td>
            </tr>
			<?php if(is_array($vo["sub"])): $i = 0; $__LIST__ = $vo["sub"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr id="tr<?php echo ($v["id"]); ?>" class="height">
              ...
              <td>
                ...
                <a href="javascript:void(0)"  onclick="iframe('<?php echo U('Admin/Ads/ads_edit',array('id'=>$v['id']));?>')"  class=" button button-small bg-sub" >修改</a>
              </td>
...
layer.confirm('确定要删除吗？', {
   ...
   $.ajax({
			type: 'POST',
			url: '<?php echo U('/Admin/Ads/ads_del');?>',
			data:{
				id:id
			},
			dataType: "json",
