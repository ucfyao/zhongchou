...
<title>约谈管理 - 会员中心芝麻乐开源众筹系统</title>
...
<div class="x12 padding">
	<div class="x12 text-center padding-top padding-bottom border-bottom border-top margin-top">
		<a href="<?php echo U('Item/interview',array('itemid'=>$_GET['itemid']));?>" class='x1 border-right <?php if($_GET['status'] == ''): ?>text-red<?php endif; ?>'>全部</a>
		<a href="<?php echo U('Item/interview',array('status'=>0,'itemid'=>$_GET['itemid']));?>" class='x1 border-right <?php if($_GET['status'] == '0'): ?>text-red<?php endif; ?>'>待约谈</a>
		<a href="<?php echo U('Item/interview',array('status'=>1,'itemid'=>$_GET['itemid']));?>" class='x1 border-right <?php if($_GET['status'] == '1'): ?>text-red<?php endif; ?>'>已约谈</a>
		<a href="<?php echo U('Item/interview',array('status'=>2,'itemid'=>$_GET['itemid']));?>" class='x1 border-right <?php if($_GET['status'] == 2): ?>text-red<?php endif; ?>'>已拒绝</a>
	</div>
</div>
<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$zml): $mod = ($i % 2 );++$i;?><div class="x12 bg padding">
		<div class="x12 bg-white padding height-big">
			<span class="float-left padding-big-left padding-big-right  border-right"><?php echo ($zml["item_name"]); ?></span>
			<span class="float-left padding-big-left padding-big-right text-small border-right">约谈人：<?php echo ($zml["user_name"]); ?></span>
			<span class="float-left padding-big-left padding-big-right  text-small border-right">提交时间：<?php echo (date("Y-m-d h:i:s",$zml["time"])); ?></span>
			<?php if(!$zml['status']): ?><button onClick="sub('<?php echo ($zml["id"]); ?>','<?php echo ($zml["user_name"]); ?>',2)" class="button float-right radius-none bg-main icon-times"> 拒绝约谈</button>
				<button onClick="sub('<?php echo ($zml["id"]); ?>','<?php echo ($zml["user_name"]); ?>',1)" class="button float-right radius-none bg-yellow icon-check  margin-right"> 约谈</button>
				<?php else: ?>
				<span class="text-red float-right"><?php echo ($zml["interview_name"]); ?></span><?php endif; ?>
		</div>
		<div class="x12 bg-white padding border-top text-small"><span class="text-yellow text padding-left">内容：</span><?php echo ($zml["content"]); ?></div>
...
<script>
	function sub(id,user_name,status){
		...
		layer.confirm(str, {
		    ...
		    $.post("/index.php/User/Item/intervies_status", {
			 	id: id,
			 	status:status
			},function(d){
				if (d.status == 1) {
