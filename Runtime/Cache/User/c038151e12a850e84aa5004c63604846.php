...
<?php if(is_array($collect)): $i = 0; $__LIST__ = $collect;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$zml): $mod = ($i % 2 );++$i;?><div class="x12 bg padding">
		...
		<div class="x9 bg padding bg-white border-left height-big">
			<div><a href="<?php echo U('Home/Item/Info',array('id'=>$zml['id']));?>" class="x6 text-big"><?php echo ($zml["item_name"]); ?></a><span class="x6 text-right"><button class="button radius-none bg-red"><?php echo ($zml["progress_name"]); ?>中</button><button onClick="collect('<?php echo ($zml["id"]); ?>','<?php echo ($zml["item_name"]); ?>')" class="button radius-none bg-yellow margin-left">取消收藏</button></span></div>
...
<script>
	function collect(id,itemname){
		layer.confirm('您是否要取消《'+itemname+'》的收藏', {
		    ...
		    $.post("/index.php/User/Investor/collect_del", {
			 	id: id,
			},function(d){
				if (d.status == 1) {
					layer.open({
