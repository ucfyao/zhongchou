...
<title>我领投的项目 - 会员中心芝麻乐开源众筹系统</title>
...
<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$zml): $mod = ($i % 2 );++$i;?><div class="x12 bg padding">
		<div class="x3 padding  bg-white">
			<img src="<?php echo ($zml["cover_img"]); ?>" alt="<?php echo ($zml["item_info"]["name"]); ?>" class="x12" height="94">
		</div>
		<div class="x9 bg padding bg-white border-left height-big">
			<div>
				<span class="x6 text-big"><?php echo ($zml["item_name"]); ?></span>
				<span class="x6 text-right text-red text-small">领投时间：<?php echo (date("Y-m-d H:i:s",$zml["time"])); ?></span>
			</div>
			<div class="x12 text-gray margin-small-top text-small">
					<span class="x12">领投责任: <span class="text-red"> <?php echo ($zml["do_what"]); ?></span></span>
			</div>
			<div class="x12 text-gray text-small">
					<span class="x12">我认购的金额: <span class="text-red"> ￥<?php echo ($zml["money"]); ?></span></span>
			</div>
		</div>
