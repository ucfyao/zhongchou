...
<div class="x12 padding">
	<div class="x12 text-center padding-top padding-bottom border-bottom border-top margin-top">
		<a href="<?php echo U('User/Funds/payment_details');?>" class='x1 border-right <?php if(($_GET['status'] != '0') AND ($_GET['status'] != 1)): ?>text-red<?php endif; ?>'>全部</a>
		<a href="<?php echo U('User/Funds/payment_details',array('status'=>1));?>" class='x1 border-right <?php if($_GET['status'] == 1): ?>text-red<?php endif; ?>'>成功</a>
		<a href="<?php echo U('User/Funds/payment_details',array('status'=>0));?>" class='x1 border-right <?php if($_GET['status'] == '0'): ?>text-red<?php endif; ?>'>未完成</a>
	</div>
</div>
<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$zml): $mod = ($i % 2 );++$i;?><div class="x12 bg padding">
		<div class="x12 bg-white padding height-big">
			<div class="x7">
				<span class="float-left padding-big-left padding-big-right  border-right">订单号：<?php echo ($zml["out_trade_no"]); ?></span>
				<span class="float-left padding-big-left padding-big-right text-small border-right">类型：<?php echo ($zml["pay_type_name"]); ?></span>
				<span class="float-left padding-big-left padding-big-right text-small border-right"><span class="text-red">￥<?php echo ($zml["money"]); ?></span></span>
			...
			<div class="x5">
				...
				<div class="x4 text-center" style="margin-top:16px">
					<?php if($zml['status'] == 1): ?><span class="text-big text-red">成功</span>
						<?php else: ?>
						<span class="text-big text-red">未完成...</span><?php endif; ?>
				</div>
			</div>
		</div>
